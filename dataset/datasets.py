...
import numpy as np
import torch
from lmdb_data_no_monai import lmdb_io_no_monai
...
def get_loader_lmdb(args, train_transforms_my, val_transforms_my=None):
    # GLOBAL_RANDOM_STATE = np.random.RandomState(47)

    val_transforms_my = None

    train_dataset = lmdb_io_no_monai.Dataset_3D(input_dir=args.train_dir, num_multiphase=args.num_multiphase, roi_z=args.roi_z,
                                       out_phase_id=args.phase_id, json_path=args.json_path, dataset_type='train',
                                       transforms=train_transforms_my)
    train_sampler = Sampler(train_dataset) if args.distributed else None

    val_dataset = lmdb_io_no_monai.Dataset_3D(input_dir=args.val_dir, num_multiphase=args.num_multiphase, roi_z=args.roi_z,
                                       out_phase_id=args.phase_id, json_path=args.json_path, dataset_type='val',
                                       transforms=val_transforms_my)
    val_sampler = Sampler(val_dataset) if args.distributed else None
...
def get_loader_lmdb_2D(args):
    train_transforms_my = transforms_v2.Compose_my([
        # transforms_v2.Normalize(min_value=args.a_min, max_value=args.a_max, clip_min=args.b_min, clip_max=args.b_max),
        transforms_v2.Resize((512, 512)),
        # transforms_v2.RandomContrast(np.random.RandomState(47)),
        # transforms_v2.RandomCrop_xy(output_size=[args.roi_x, args.roi_y, args.roi_z], num_sample=args.num_sample, pos_ratio=2, neg_ratio=1,
        #            random_state=np.random.RandomState(47)),
        transforms_v2.RandomFlip(random_state=np.random.RandomState(48), type='2D', axis_prob=0.2),
        transforms_v2.RandomRotate90(random_state=np.random.RandomState(49), type='2D', execution_probability=0.2)
    ])

    # train_transforms_my = None
    val_transforms_my = None
    # val_transforms_my = transforms_v2.Compose_my([
        # transforms_v2.Normalize(min_value=args.a_min, max_value=args.a_max, clip_min=args.b_min, clip_max=args.b_max),
        # transforms_v2.Resize((512, 512))
    # ])

    train_dataset = lmdb_io_no_monai.Dataset_2D(input_dir=args.train_dir, num_multiphase=args.num_multiphase,out_phase_id=[2],
                                                json_path=args.json_path, dataset_type='train', transforms=train_transforms_my, type='2D_ratio')
    train_sampler = Sampler(train_dataset) if args.distributed else None


    val_dataset = lmdb_io_no_monai.Dataset_3D(input_dir=args.val_dir, num_multiphase=args.num_multiphase, out_phase_id=[2],
                                              json_path=args.json_path, dataset_type='val', transforms=val_transforms_my)
    val_sampler = Sampler(val_dataset) if args.distributed else None
...
def get_loader_lmdb_2D_512(args):
    # train_transforms_my = transforms_v2.Compose_my([
        # transforms_v2.Normalize(min_value=args.a_min, max_value=args.a_max, clip_min=args.b_min, clip_max=args.b_max),
        # transforms_v2.Resize((512, 512)),
        # transforms_v2.RandomContrast(np.random.RandomState(47)),
        # transforms_v2.RandomCrop_xy(output_size=[args.roi_x, args.roi_y, args.roi_z], num_sample=args.num_sample, pos_ratio=2, neg_ratio=1,
        #            random_state=np.random.RandomState(47)),
        # transforms_v2.RandomFlip(random_state=np.random.RandomState(48), axis_prob=0.2),
        # transforms_v2.RandomRotate90(random_state=np.random.RandomState(49), execution_probability=0.2)
    # ])
...
def get_loader(args):
    ...
    train_transform = transforms.Compose(
        [
            ...
            transforms.SpatialPadd(keys=["image", "label"], spatial_size=(-1, -1, args.roi_z), method="end"),
    ...
    val_transform = transforms.Compose(
        [
            ...
            transforms.SpatialPadd(keys=["image"], spatial_size=(-1, -1, args.roi_z)),
...
def get_loader_weight(args):
    ...
    train_transform = transforms.Compose(
        [
            ...
            transforms.SpatialPadd(keys=["image", "label", "weight"], spatial_size=(-1, -1, args.roi_z), method="end"),
            transforms.RandCropByPosNegLabeld(  # 按照特定阴性阳性比例裁剪子图
...
def get_loader_mp_weight(args):
    ...
    train_transform = transforms.Compose(
        [
            ...
            transforms.SpatialPadd(keys=["image", "label", "weight"], spatial_size=(-1, -1, args.roi_z)),
            transforms.RandCropByPosNegLabeld(  # 按照特定阴性阳性比例裁剪子图
                keys=["image", "label", "weight"],
                label_key="label",
                spatial_size=(args.roi_x, args.roi_y, args.roi_z),
