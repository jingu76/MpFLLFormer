...
def load(path):
    ...
    tumor_mask[tumor_mask==1] = 0  # ignore liver
    return tumor_mask
...
class MetricValues:
    ...
    def print_result(self):
        ...
        for idx, i in enumerate(CLASSES):
            dsc_i = safe_divide(self.dsc[idx], self.case_num)
            dsc_msg += f'dsc({CLASS_NAME[idx]}):{dsc_i:.4f} '
        print(
            dsc_msg + f'voe:{tumor_voe:.4f} sensitivity:{sensitivity:.4f} precision:{precision:.4f} f2_score:{tumor_f2_score:.4f}  rvd:{rvd:.4f}'
        )
...
def run(args, filename):
    ...
    for idx, i in enumerate(CLASSES):
        ...
        metric.dsc[idx] = cal_dsc(pre_i, gt_i)
    return metric
