...
from dataset.datasets import get_loader_val
...
def main_worker(gpu, args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(loader):
            val_inputs, val_labels = (batch["image"].to(args.rank, non_blocking=True).contiguous().float(),
                                      batch["label"].to(args.rank, non_blocking=True).contiguous().long())
            ...
            logger.info("{}/{} {}: {}".format(i, len(loader), img_name, mean_dice))
            dice_sum += mean_dice
            data_num += 1
            if args.save_output:
                nib.save(
