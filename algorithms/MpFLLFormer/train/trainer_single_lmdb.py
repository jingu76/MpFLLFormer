...
from utils.utils import AverageMeter, distributed_all_gather, reduce_by_weight, resample_3d, cal_dice, unSpatialPad_v2, unSpatialPad

from utils.utils import info_if_main


def train_epoch(model, loader, optimizer: torch.optim.Optimizer, scaler, epoch, loss_func, args):
    model.train()
    ...
    for idx, batch_data in enumerate(loader):
        ...
        target = rearrange(labels, "b n c w h d -> (b n) c w h d").to(args.rank, non_blocking=True).contiguous().long()
...
def val_epoch_v2(model, loader, epoch, args, model_inferer=None):
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
            ...
            target = labels.to(args.rank, non_blocking=True).contiguous().long()
